...
public class BundleItemScreen extends HandledScreen<BundleItemScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float deltaTicks, int mouseX, int mouseY) {
        context.drawTexture(RenderLayer::getGuiTextured, CONTAINER_TEXTURE, this.x, this.y, 0.0F, 0.0F, this.backgroundWidth, ROWS_COUNT * 18 + 17, 256, 256);
        context.drawTexture(RenderLayer::getGuiTextured, CONTAINER_TEXTURE, this.x, this.y + ROWS_COUNT * 18 + 17, 0.0F, 126.0F, this.backgroundWidth, 96, 256, 256);
        this.drawScrollbarBackground(context);
        int i = this.x + 156;
        int j = this.y + 18;
        int k = j + 88;
        context.drawGuiTexture(RenderLayer::getGuiTextured, SCROLLER_TEXTURE, i, j + (int) ((k - j - SCROLLBAR_HEIGHT) * this.scrollPosition), SCROLLBAR_WIDTH, SCROLLBAR_HEIGHT);
    }

    public void drawScrollbarBackground(DrawContext context){
        int i = this.x + 8 + COLUMNS_COUNT * 18 - 1;
        int j = this.y + 18 - 1;
        context.drawTexture(RenderLayer::getGuiTextured, SCROLLBAR_BACKGROUND_TEXTURE, i, j, 170, 17, 18, 72, 256, 256);
        context.drawTexture(RenderLayer::getGuiTextured, SCROLLBAR_BACKGROUND_TEXTURE, i, j + 72, 170, 111, 18, 18, 256, 256);
    }
