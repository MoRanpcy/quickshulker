...
public class BundleContainer extends SimpleContainer {
    ...
    public BundleContents getBundleContentsByItems(){
        return new BundleContents(this.items.stream().filter(itemStack -> !itemStack.isEmpty()).map(ItemStackTemplate::fromNonEmptyStack).toList());
    }
    ...
    public void setChanged() {
        ...
        this.items.stream().filter(stack -> !stack.isEmpty()).map(ItemStack::copy).forEach(stack -> builder.add(ItemStackTemplate.fromNonEmptyStack(stack)));
