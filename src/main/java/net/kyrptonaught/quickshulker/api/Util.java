...
public class Util {
    ...
    public static void openItem(Player player, int invSlot, int playerInvIndex) {
        if (QuickShulker.getConfig().rightClickClose && playerInvIndex == ((ItemInventoryContainer) player.containerMenu).getUsedSlotInPlayerInv()) {
        ...
        if (qsData != null) {
            ...
            if(QuickShulker.getConfig().playSound) playSound(player, qsData.getSound(stack, true));
